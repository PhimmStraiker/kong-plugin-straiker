...
konggw-bedrock-on() {  # Claude Code -> shared gateway -> Bedrock (needs your own Bedrock token)
  if [ -z "${KONGGW_KEY:-}" ]; then echo "set KONGGW_KEY first"; return 1; fi
...
konggw-status() {
  echo "ANTHROPIC_BASE_URL=${ANTHROPIC_BASE_URL:-<unset>}  BEDROCK=${CLAUDE_CODE_USE_BEDROCK:-0}  key=$([ -n "${KONGGW_KEY:-}" ] && echo set || echo MISSING)"
}
